# 👋 Hi, I'm Alejandro (aka @Pirueto)

**Software Engineer | Machine Learning Enthusiast | Cloud & DevOps Curious**

I love building real-world solutions using clean architecture, automation, and modern tooling.  
Currently exploring **ML engineering**, containerized workflows, and scalable backend systems.

---

## 🚀 About Me
- 🔭 I’m currently working on ML pipeline automation and model deployment.
- 🌱 Exploring **PyTorch**, **MLOps best practices**, and **distributed cloud systems**.
- 💼 Experience with multiple **ML frameworks**, Docker, Kafka, Protobuf, and microservices.
- 🌍 Based in Spain and open to relocation.

---

## 🛠️ Tech Stack

**Languages:**  
Mainly `Python` · `Bash`

**ML / Data:**  
`PyTorch` · `TensorFlow` · `scikit-learn` · `Pandas` · `Numpy`

**DevOps & Tools:**  
`Docker` · `Kubernetes` · `Kafka` · `Protobuf` · `GitHub Actions` & `Gitlab CI`

---

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Pirueto&show_icons=true&hide_border=true&bg_color=303446&text_color=c6d0f5&icon_color=ca9ee6&title_color=81c8be)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Pirueto&layout=compact&hide_border=true&bg_color=303446&text_color=c6d0f5&icon_color=ca9ee6&title_color=81c8be)

---

## 📫 Contact
- ✉️ Email: alexpirueto@gmail.com  
- 💼 LinkedIn: [Alejandro Sánchez](https://www.linkedin.com/in/alejandrosanchezalocen/)
- 🌐 Portfolio: _Coming soon..._

---

⭐️ *Feel free to check out my repositories or reach out if you want to collaborate!*

<!---
Pirueto/Pirueto is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
